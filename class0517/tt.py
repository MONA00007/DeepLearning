...
class ContextEncoder():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50):
        ...
        if os.path.exists('class0517/saved_model/discriminator_weights.hdf5'
                          ) and os.path.exists(
                'class0517/saved_model/generator_weights.hdf5'):
            self.discriminator.load_weights(
                'class0517/saved_model/discriminator_weights.hdf5')
        ...
        list = glob.glob(r'class0517/train_images/arch/*.jpg')
        for ll in list:
            im = cv2.imread(ll)
        ...
        for epoch in range(epochs):
            ...
            # d_loss[0]判别器损失, d_loss[1]准确率
            # g_loss[0]联合模型的重建损失, g_loss[1]联合模型的对抗损失
    ...
    def save_model(self):
        ...
        def save(model, model_name):
            model_path = "class0517/saved_model/%s.json" % model_name
            weights_path = "class0517/saved_model/%s_weights.hdf5" % model_name
    ...
    def sample_images(self, epoch, imgs):
        ...
        for i in range(c):
            ...
            axs[2, i].axis('off')
        fig.savefig("class0517/images/%d.png" % epoch, dpi=256)
        plt.close()
