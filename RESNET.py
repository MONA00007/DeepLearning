...
def identity_block1(X, f, channels):
    '''
    卷积块--(等值函数)--卷积块
    '''
    F1, F2, F3 = channels
    ...
    X = keras.layers.Activation('relu')(X)
    # 块 3
    X = keras.layers.Conv2D(filters=F3, kernel_size=(3, 3), strides=(1, 1), padding='valid')(X)
    X = keras.layers.BatchNormalization(axis=3)(X)
    # 跳跃连接
    X = keras.layers.Add()([X, X_shortcut])
    X = keras.layers.Activation('relu')(X)
...
def convolutional_block1(X, f, channels, s=2):
    '''
    卷积块--(卷积块)--卷积块
    '''
    F1, F2, F3 = channels
...
X = keras.layers.BatchNormalization(axis=3)(X)
X = keras.layers.Activation('relu')(X)
X = keras.layers.MaxPooling2D((2, 2), strides=(2, 2))(X) #64,64,64
# 残差块1
...
X = identity_block(X, 3, [512, 512, 2048])
# 残差块5
X = convolutional_block1(X, 3, [2048, 2048, 1024])
X = identity_block1(X, 3, [2048, 2048, 1024])
X = identity_block1(X, 3, [2048, 2048, 1024])
# 残差块6
